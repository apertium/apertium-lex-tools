...
struct Z{
  D k,q,l;Z(D a=0,D b=-1,D c=0):k(a),q(c),l((b<0)?1:log(b)){};
};
...
int main(int argc,char**av)
{
  ...
  D TRN=0.0, TST=0.0;
  bool qt=0, initmu=0;
  vector<pair<string,D> > s2f;vector<Z> z;
  ifstream *muf=0;
  int mfc=-2, kfl=0;
  ...
  for(size_t n=0;n<Es;++n){const event&en=E[n].first;
    ...
    do { 
      old_l=l;l=0.0;w=0.0;lt=0.0;wt=0.0;double wx=0.0,wtx=0.0,lx=0,ltx=0;
